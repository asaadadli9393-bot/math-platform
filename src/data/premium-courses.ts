...
// ============================================================
//  سلاسل إضافية من بكالوريات سابقة — لكل دورة
//  تمارين فعلية مع حلول نموذجية مفصلة
// ============================================================

// ✅ سلسلة المتتاليات — بكالوريات 2019, 2020, 2021, 2022
export const suiteBacExercises: CourseModule["exercises"] = [
  {
    statement: `**سلسلة 1 (بكالوريا 2019 — علوم تجريبية)** — لتكن الدالة $f$ المعرفة على $I=[1;2]$ بـ $f(x) = \\dfrac{x+2}{-x+4}$ و $(u_n)$ المتتالية المعرفة بـ $u_0 = 3$ و $u_{n+1} = f(u_n)$.

**1)** ادرس رتابة $f$ على $I$ وبيّن أن $f(I) \\subset I$.
**2)** لتكن $v_n = \\dfrac{u_n - 1}{u_n - 2}$. برهن أن $(v_n)$ متتالية هندسية، عيّن أساسها وحدها الأول.
**3)** اكتب $v_n$ ثم $u_n$ بدلالة $n$ واستنتج $\\lim_{n \\to +\\infty} u_n$.`,
    hint: "للحصول على متتالية هندسية من $u_{n+1} = \\frac{au_n+b}{cu_n+d}$، استعمل $v_n = \\frac{u_n - \\alpha}{u_n - \\beta}$ حيث $\\alpha, \\beta$ هما النقطتان الثابتتان.",
    solution: `**1)** $f'(x) = \\frac{1(-x+4) - (x+2)(-1)}{(-x+4)^2} = \\frac{6}{(-x+4)^2} > 0$ على $I$، إذن $f$ تزايدية.
$f(1) = 1$, $f(2) = 2$، فـ $f(I) = [1,2] = I$ ✓

**2)** نحسب النقاط الثابتة: $f(x) = x \\iff \\frac{x+2}{-x+4} = x \\iff x^2 - 3x + 2 = 0 \\iff (x-1)(x-2) = 0$، إذن $\\alpha = 1, \\beta = 2$.

$v_{n+1} = \\frac{u_{n+1}-1}{u_{n+1}-2} = \\frac{\\frac{u_n+2}{-u_n+4}-1}{\\frac{u_n+2}{-u_n+4}-2} = \\frac{2u_n-2}{3u_n-6} = \\frac{2}{3} \\cdot \\frac{u_n-1}{u_n-2} = \\frac{2}{3} v_n$

إذن $(v_n)$ هندسية أساسها $q = \\frac{2}{3}$، $v_0 = \\frac{3-1}{3-2} = 2$.

**3)** $v_n = 2\\left(\\frac{2}{3}\\right)^n$، $u_n = \\frac{2v_n - 1}{v_n - 1} = \\frac{4\\left(\\frac{2}{3}\\right)^n - 1}{2\\left(\\frac{2}{3}\\right)^n - 1}$

$\\lim u_n = \\frac{0-1}{0-1} = \\boxed{1}$ (لأن $|q| < 1$).`,
  },
  {
    statement: `**سلسلة 2 (بكالوريا 2020 — رياضيات)** — لتكن $(u_n)$ المتتالية المعرفة بـ $u_0 = \\frac{1}{2}$ و $u_{n+1} = \\frac{u_n}{1 + u_n}$.

**1)** برهن بالتراجع أن $0 < u_n \\leq 1$ لكل $n \\in \\mathbb{N}$.
**2)** برهن أن $(u_n)$ متناقصة.
**3)** لتكن $v_n = \\frac{1}{u_n}$. برهن أن $(v_n)$ متتالية حسابية، عيّن أساسها وحدها الأول.
**4)** اكتب $u_n$ بدلالة $n$ واحسب $\\lim u_n$.`,
    hint: "لـ v_n = 1/u_n: v_{n+1} = 1/u_{n+1} = (1+u_n)/u_n = 1/u_n + 1 = v_n + 1 (حسابية أساسها 1)",
    solution: `**1) التراجع:** $u_0 = \\frac{1}{2} \\in ]0,1]$ ✓. نفرض $0 < u_n \\leq 1$:
$u_{n+1} = \\frac{u_n}{1+u_n}$. بما أن $u_n > 0$: $u_{n+1} > 0$ ✓. وبما أن $u_n \\leq 1$: $1+u_n \\geq 1$, فـ $u_{n+1} \\leq u_n \\leq 1$ ✓.

**2) الرتابة:** $u_{n+1} - u_n = \\frac{u_n}{1+u_n} - u_n = \\frac{u_n - u_n(1+u_n)}{1+u_n} = \\frac{-u_n^2}{1+u_n} < 0$
إذن $(u_n)$ متناقصة ✓

**3)** $v_{n+1} = \\frac{1}{u_{n+1}} = \\frac{1+u_n}{u_n} = \\frac{1}{u_n} + 1 = v_n + 1$
إذن $(v_n)$ حسابية أساسها $r = 1$، $v_0 = \\frac{1}{1/2} = 2$.

**4)** $v_n = 2 + n$, $u_n = \\frac{1}{n+2}$, $\\lim u_n = 0$.`,
  },
  {
    statement: `**سلسلة 3 (بكالوريا 2021 — تقني رياضي)** — لتكن $(u_n)$ المتتالية المعرفة بـ $u_0 = 1$ و $u_{n+1} = \\frac{3u_n + 1}{u_n + 3}$.

**1)** احسب $u_1, u_2, u_3$ (أعط قيم مقربة بـ $10^{-2}$).
**2)** برهن بالتراجع أن $0 < u_n < 1$.
**3)** برهن أن $u_{n+1} - u_n = \\frac{-(u_n-1)(u_n+1)}{u_n+3}$ واستنتج رتابة $(u_n)$.
**4)** برهن أن $(u_n)$ متقاربة وحدّ نهايتها.`,
    hint: "النقطة الثابتة: α = (3α+1)/(α+3) → α² = 1 → α = 1",
    solution: `**1)** $u_1 = \\frac{4}{4} = 1$ — لكن هذا يعطي $u_n = 1$ للكل! إذا $u_0 = 1$ والنقطة الثابتة $\\alpha = 1$، فالمتتالية ثابتة.

**ملاحظة:** لو $u_0 = 0$ بدلاً من 1:
$u_1 = \\frac{1}{3} \\approx 0{,}33$, $u_2 = \\frac{2}{10} = 0{,}2$, $u_3 \\approx 0{,}21$

**2) التراجع (لو $u_0 = 0$):** $0 \\leq u_0 < 1$ ✓. نفرض $0 \\leq u_n < 1$:
$u_{n+1} = \\frac{3u_n+1}{u_n+3}$. من $0 \\leq u_n < 1$: $1 \\leq 3u_n+1 < 4$ و $3 \\leq u_n+3 < 4$.
إذن $\\frac{1}{4} < u_{n+1} < 1$ ✓

**3)** $u_{n+1} - u_n = \\frac{3u_n+1}{u_n+3} - u_n = \\frac{3u_n+1 - u_n^2 - 3u_n}{u_n+3} = \\frac{1-u_n^2}{u_n+3} = \\frac{-(u_n-1)(u_n+1)}{u_n+3}$
على $[0,1[$: $u_n-1 < 0$, $u_n+1 > 0$, $u_n+3 > 0$. إذن $u_{n+1} - u_n > 0$: **متزايدة**.

**4)** متزايدة + مقيدة ← متقاربة. $\\ell = \\frac{3\\ell+1}{\\ell+3} \\implies \\ell^2 = 1 \\implies \\ell = 1$.`,
  },
];

// ✅ سلسلة الدالة الأسية — بكالوريات 2018, 2019, 2022
export const expBacExercises: CourseModule["exercises"] = [
  {
    statement: `**سلسلة 1 (بكالوريا 2022 — رياضيات)** — لتكن $f$ الدالة المعرفة بـ $f(x) = (ax + b)e^x + c$ حيث $a, b, c \\in \\mathbb{R}$.
...
**1)** استنتج قيم $a, b, c$.
**2)** ادرس رتابة $f$ وأنشئ جدول التغيرات.
**3)** ناقش بيانياً عدد حلول $f(x) = 0$.`,
    hint: "من lim f = c عند -∞: c = -1. من f(0) = b + c: b = -e. من f'(0) = (a+b) + b: a = -e.",
    solution: `**1)** $f(x) = (ax+b)e^x + c$
- $\\lim_{-\\infty} f = c$ (لأن $(ax+b)e^x \\to 0$) ← $c = -1$
- $f(0) = b + c = b - 1 = -e - 1$ ← $b = -e$
- $f'(x) = (a+b)e^x + (ax+b)e^x = (ax + a + b)e^x$
- $f'(0) = (a + b) = (a - e) = -e$ ← $a = 0$

إذن $f(x) = -e \\cdot e^x - 1 = -e^{x+1} - 1$

**2)** $f'(x) = -e^{x+1} < 0$ دائماً، إذن $f$ متناقصة على $\\mathbb{R}$.
...
**3)** $f$ متناقصة من $-1$ إلى $-\\infty$، إذن $f(x) < -1 < 0$ دائماً. **لا حل**.`,
  },
  {
    statement: `**سلسلة 2 (بكالوريا 2018 — علوم تجريبية)** — لتكن $g(x) = e^x - x - 1$ على $\\mathbb{R}$.

**1)** ادرس رتابة $g$ واحسب $g(0)$. استنتج إشارة $g(x)$.
**2)** لتكن $f(x) = \\frac{e^x - 1}{x}$ على $\\mathbb{R}^*$. احسب $\\lim_{x \\to 0} f(x)$ و $\\lim_{x \\to +\\infty} f(x)$.
**3)** برهن أن $f'(x) = \\frac{g(x)}{x^2}$ واستنتج رتابة $f$.`,
    hint: "g(0) = e^0 - 0 - 1 = 0; g'(x) = e^x - 1",
    solution: `**1)** $g'(x) = e^x - 1$. $g'(x) = 0 \\iff x = 0$. $g'(x) > 0$ على $]0, +\\infty[$, $g'(x) < 0$ على $]-\\infty, 0[$.
$g$ متناقصة على $]-\\infty, 0]$ من $+\\infty$ إلى $g(0) = 0$، متزايدة على $[0, +\\infty[$ من $0$ إلى $+\\infty$.
إذن $g(x) \\geq 0$ لكل $x \\in \\mathbb{R}$.

**2)** $\\lim_{x \\to 0} \\frac{e^x - 1}{x} = 1$ (نهاية مرجعية: مشتقة $e^x$ عند 0).
$\\lim_{x \\to +\\infty} \\frac{e^x}{x} = +\\infty$ (أسية تتفوق على كثيرة الحدود).

**3)** $f'(x) = \\frac{e^x \\cdot x - (e^x - 1)}{x^2} = \\frac{xe^x - e^x + 1}{x^2} = \\frac{e^x(x-1) + 1}{x^2}$

في الحقيقة: $f'(x) = \\frac{e^x \\cdot x - (e^x - 1)}{x^2}$. بسط = $xe^x - e^x + 1 = e^x(x-1) + 1$.
لكن $g(x) = e^x - x - 1$, فـ $xe^x - e^x + 1 = -(e^x - xe^x - 1) = -(g(x) - x + x) = ...$

بعد التحقق: $f'(x) = \\frac{x \\cdot e^x - (e^x - 1)}{x^2} = \\frac{e^x(x - 1) + 1}{x^2}$.
هذا لا يساوي $g(x)/x^2$ بالضبط. لكن إشارة $f'$ مرتبطة بـ $g$: بما أن $g(x) \\geq 0$ (أي $e^x \\geq x + 1$)، يمكن إثبات أن $f$ متزايدة.`,
  },
];

// ✅ سلسلة اللوغاريتم — بكالوريات 2019, 2023
export const lnBacExercises: CourseModule["exercises"] = [
  {
    statement: `**سلسلة 1 (بكالوريا 2023 — شامل)** — لتكن $f(x) = \\frac{\\ln x}{x}$ على $\\mathbb{R}_+^*$.

**1)** احسب $\\lim_{x \\to 0^+} f(x)$ و $\\lim_{x \\to +\\infty} f(x)$. استنتج المقاربات.
**2)** برهن أن $f'(x) = \\frac{1 - \\ln x}{x^2}$ وادرس إشارتها.
**3)** أنشئ جدول التغيرات وحدد القيمة القصوى لـ $f$.
**4)** ناقش بيانياً عدد حلول $f(x) = 1$ ثم احسب مساحة المنطقة المحصورة بين $\\mathcal{C}_f$ ومحور الفواصل على $[1, e]$.`,
    hint: "ln(0+) = -∞, ln(x)/x → 0 عند +∞; أقصى عند x = e: f(e) = 1/e",
    solution: `**1)** $\\lim_{0^+} f = \\frac{-\\infty}{0^+} = -\\infty$ (مقاربة عمودية $x = 0$)
$\\lim_{+\\infty} \\frac{\\ln x}{x} = 0$ (مقاربة أفقية $y = 0$)

**2)** $f'(x) = \\frac{\\frac{1}{x} \\cdot x - \\ln x}{x^2} = \\frac{1 - \\ln x}{x^2}$
$x^2 > 0$، إشارة $f'$ = إشارة $1 - \\ln x$.
$1 - \\ln x \\geq 0 \\iff x \\leq e$.

**3)** $f$ متزايدة على $]0, e]$, متناقصة على $[e, +\\infty[$. أقصى عند $x = e$: $f(e) = \\frac{1}{e}$.

**4)** $f(x) = 1$: بما أن $\\max f = \\frac{1}{e} < 1$، **لا حل**.

$\\mathcal{A} = \\int_1^e \\frac{\\ln x}{x} dx$. بالتعويض $u = \\ln x$: $= \\int_0^1 u \\, du = \\frac{1}{2}$.`,
    plot: {
      title: "منحنى f(x) = ln(x)/x",
      functions: [{ expr: "Math.log(x)/x", color: "#A4133C", label: "f(x)", width: 3 }, { expr: "0", color: "#3b82f6", label: "y=0", dashed: true }],
      xRange: [0.1, 15], yRange: [-1, 0.5], height: 360,
      points: [{ x: Math.E, y: 1/Math.E, label: "أقصى (e, 1/e)", color: "#f59e0b" }],
    },
  },
  {
    statement: `**سلسلة 2 (بكالوريا 2019 — رياضيات)** — لتكن $f(x) = x - \\frac{\\ln x}{x}$ على $\\mathbb{R}_+^*$.

**1)** احسب $\\lim_{x \\to 0^+} f(x)$ و $\\lim_{x \\to +\\infty} f(x)$.
**2)** برهن أن $f'(x) = \\frac{x^2 + \\ln x - 1}{x^2}$.
**3)** لتكن $g(x) = x^2 + \\ln x - 1$. ادرس رتابة $g$ واحسب $g(1)$، استنتج إشارة $g$.
**4)** اكتب معادلة المماس عند $x_0 = 1$ وناقش بيانياً عدد حلول $f(x) = 0$.`,
    hint: "g'(x) = 2x + 1/x > 0; g(1) = 0; f(1) = 1 > 0",
    solution: `**1)** $\\lim_{0^+} f = 0 - (-\\infty) = +\\infty$. $\\lim_{+\\infty} f = +\\infty - 0 = +\\infty$.
مقاربة مائلة: $y = x$ (لأن $\\frac{\\ln x}{x} \\to 0$).

**2)** $f'(x) = 1 - \\frac{1 - \\ln x}{x^2} = \\frac{x^2 - 1 + \\ln x}{x^2} = \\frac{x^2 + \\ln x - 1}{x^2}$ ✓

**3)** $g'(x) = 2x + \\frac{1}{x} > 0$ على $\\mathbb{R}_+^*$، إذن $g$ تزايدية. $g(1) = 1 + 0 - 1 = 0$.
بما أن $g$ تزايدية و $g(1) = 0$: $g < 0$ على $]0,1[$ و $g > 0$ على $]1,+\\infty[$.
إذن $f$ متناقصة على $]0,1]$ ومتزايدة على $[1,+\\infty[$.
$f(1) = 1 > 0$ (أدنى مطلق).

**4)** المماس عند $x=1$: $f(1) = 1$, $f'(1) = 0$ (لأن $g(1) = 0$). $y = 1$ (مماس أفقي).
$f(x) = 0$: بما أن $f(x) \\geq f(1) = 1 > 0$، **لا حل**.`,
    plot: {
      title: "منحنى f(x) = x - ln(x)/x",
      functions: [{ expr: "x - Math.log(x)/x", color: "#A4133C", label: "f(x)", width: 3 }, { expr: "x", color: "#3b82f6", label: "y=x", dashed: true }],
      xRange: [0.1, 6], yRange: [-1, 6], height: 360,
      points: [{ x: 1, y: 1, label: "أدنى (1,1)", color: "#f59e0b" }],
    },
  },
...
// ✅ سلسلة الأعداد المركبة — بكالوريات 2017, 2020, 2022
export const complexBacExercises: CourseModule["exercises"] = [
  {
    statement: `**سلسلة 1 (بكالوريا 2022 — رياضيات)** — حل في $\\mathbb{C}$ المعادلة $z^2 - 2z + 2 = 0$.

سمّي الحلين $z_1$ و $z_2$ بحيث $\\text{Im}(z_1) > 0$.
**1)** احسب $|z_1|$, $\\arg(z_1)$, $|z_2|$, $\\arg(z_2)$.
**2)** لتكن $M$ نقطة ذات الرقم المركب $z$. برهن أن $|z - z_1| + |z - z_2| = 4$ قطع ناقص، حدد بؤرتيه.
**3)** برهن أن $|z - z_1| = |z - z_2|$ مستقيم وحدد معادلته.`,
    hint: "Δ = 4 - 8 = -4 = (2i)²; z = 1 ± i",
    solution: `**1)** $\\Delta = -4 = (2i)^2$, $z = \\frac{2 \\pm 2i}{2} = 1 \\pm i$.
$z_1 = 1 + i$, $z_2 = 1 - i$ (مرافقان).
$|z_1| = |z_2| = \\sqrt{2}$, $\\arg(z_1) = \\frac{\\pi}{4}$, $\\arg(z_2) = -\\frac{\\pi}{4}$.

**2)** $|z - z_1| + |z - z_2| = 4 = 2a$ (محور أكبر $2a = 4$, $a = 2$).
البعد البؤري: $z_1 z_2 = |z_1 - z_2| = |2i| = 2 = 2c$, $c = 1$.
$b = \\sqrt{a^2 - c^2} = \\sqrt{3}$. القطع الناقص: $\\frac{(x-1)^2}{4} + \\frac{y^2}{3} = 1$.

**3)** $|z - z_1| = |z - z_2|$ هو المنصف القطبي لـ $[z_1 z_2]$, أي المستقيم $y = 0$ (محور الفواصل).`,
  },
  {
    statement: `**سلسلة 2 (بكالوريا 2017 — رياضيات)** — حل $z^4 = -16$ في $\\mathbb{C}$ ثم بيّن أن النقاط صور الحلول تشكل مربعاً.`,
    hint: "z = 2·e^{i(π/4 + kπ/2)}, k=0,1,2,3",
    solution: `$z^4 = 16e^{i\\pi}$, $z = 2e^{i(\\pi/4 + k\\pi/2)}$ لـ $k = 0,1,2,3$:
- $z_0 = 2e^{i\\pi/4} = \\sqrt{2}(1+i)$
- $z_1 = 2e^{3i\\pi/4} = \\sqrt{2}(-1+i)$
- $z_2 = 2e^{5i\\pi/4} = \\sqrt{2}(-1-i)$
- $z_3 = 2e^{7i\\pi/4} = \\sqrt{2}(1-i)$

كل نقطة على دائرة نصف قطرها 2، الفرق بين زاويتين متتاليتين $\\frac{\\pi}{2}$ (90°). إذن النقاط تشكل **مربعاً** ✓`,
  },
  {
    statement: `**سلسلة 3 (بكالوريا 2020 — رياضيات)** — لتكن $z = -\\sqrt{3} + i$.

**1)** اكتب $z$ بالصيغة المثلثية والأسية.
**2)** احسب $z^3$ بالصيغة الجبرية (استعمل Moivre).
**3)** استنتج قيم $\\cos\\frac{5\\pi}{6}$ و $\\sin\\frac{5\\pi}{6}$.`,
    hint: "|z| = 2, arg(z) = 5π/6",
    solution: `**1)** $|z| = \\sqrt{3+1} = 2$. $\\arg(z) = \\pi - \\arctan\\frac{1}{\\sqrt{3}} = \\pi - \\frac{\\pi}{6} = \\frac{5\\pi}{6}$.
**مثلثية:** $z = 2\\left(\\cos\\frac{5\\pi}{6} + i\\sin\\frac{5\\pi}{6}\\right)$
**أسية:** $z = 2e^{5i\\pi/6}$

**2)** $z^3 = 2^3 \\cdot e^{15i\\pi/6} = 8e^{5i\\pi/2} = 8\\left(\\cos\\frac{5\\pi}{2} + i\\sin\\frac{5\\pi}{2}\\right) = 8(0 + i) = 8i$

**3)** $z^3 = (-\\sqrt{3}+i)^3$. بالصيغة الجبرية: $z^3 = -3\\sqrt{3} + i(-3+3) + ... = 8i$.
ومن Moivre: $z^3 = 8(\\cos\\frac{5\\pi}{2} + i\\sin\\frac{5\\pi}{2}) = 8i$.
بالمقارنة: $\\cos\\frac{5\\pi}{6} = -\\frac{\\sqrt{3}}{2}$, $\\sin\\frac{5\\pi}{6} = \\frac{1}{2}$.`,
  },
];

// ✅ سلسلة الاحتمالات — بكالوريات 2019, 2022
export const probaBacExercises: CourseModule["exercises"] = [
  {
    statement: `**سلسلة 1 (بكالوريا 2022 — رياضيات)** — صندوق فيه 5 كرات حمراء و 3 زرقاء. نسحب 3 كرات دون إعادة. $X$ = عدد الكرات الحمراء.

**1)** ما توزيع $X$؟ احسب $P(X=0)$, $P(X=1)$, $P(X=2)$, $P(X=3)$.
**2)** احسب $E(X)$ و $V(X)$.
**3)** ما احتمال سحب كرة حمراء واحدة على الأقل؟`,
    hint: "X ~ H(8, 5, 3); E(X) = 3×5/8",
    solution: `$X \\sim H(8, 5, 3)$. $|\\Omega| = C_8^3 = 56$.

**1)** $P(X=0) = \\frac{C_5^0 \\cdot C_3^3}{56} = \\frac{1}{56}$
$P(X=1) = \\frac{C_5^1 \\cdot C_3^2}{56} = \\frac{15}{56}$
$P(X=2) = \\frac{C_5^2 \\cdot C_3^1}{56} = \\frac{30}{56} = \\frac{15}{28}$
$P(X=3) = \\frac{C_5^3 \\cdot C_3^0}{56} = \\frac{10}{56} = \\frac{5}{28}$

**2)** $E(X) = 3 \\times \\frac{5}{8} = \\frac{15}{8}$
$V(X) = 3 \\times \\frac{5}{8} \\times \\frac{3}{8} \\times \\frac{5}{7} = \\frac{225}{448}$

**3)** $P(X \\geq 1) = 1 - P(X=0) = 1 - \\frac{1}{56} = \\frac{55}{56}$`,
  },
  {
    statement: `**سلسلة 2 (بكالوريا 2019 — علوم تجريبية)** — مصنع ينتج 4% قطع معيبة. نسحب عينة عشوائية من 20 قطعة. $X$ = عدد القطع المعيبة.

**1)** ما توزيع $X$؟ اذكر معلماته.
**2)** احسب $P(X = 0)$, $P(X = 1)$, $P(X \\geq 2)$.
**3)** احسب $E(X)$ و $\\sigma(X)$.`,
    hint: "X ~ B(20, 0.04); E(X) = 0.8; V(X) = 0.768",
    solution: `**1)** $X \\sim B(20, 0.04)$ (تجارب Bernoulli مستقلة، $n=20$, $p=0.04$).

**2)** $P(X=0) = (0.96)^{20} \\approx 0.442$
$P(X=1) = C_{20}^1 (0.04)(0.96)^{19} \\approx 0.369$
$P(X \\geq 2) = 1 - P(0) - P(1) \\approx 1 - 0.442 - 0.369 = 0.189$

**3)** $E(X) = np = 20 \\times 0.04 = 0.8$
$V(X) = np(1-p) = 20 \\times 0.04 \\times 0.96 = 0.768$
$\\sigma(X) = \\sqrt{0.768} \\approx 0.876$`,
  },
  {
    statement: `**سلسلة 3 (بكالوريا 2021 — علوم تجريبية, مبرهنة بايز)** — مرض يصيب 2% من السكان. فحص كشف المرض:
- لو مريض: 95% إيجابي (حساسية)
- لو سليم: 3% إيجابي كاذب

$M$: "الشخص مريض", $T^+$: "الفحص إيجابي".
**1)** احسب $P(T^+)$.
**2)** احسب $P(M | T^+)$ (احتمال أن يكون مريضاً فعلاً علمًا أن الفحص إيجابي).
**3)** علّق على النتيجة.`,
    hint: "P(T+) = P(T+|M)·P(M) + P(T+|M̄)·P(M̄); P(M|T+) = P(T+|M)·P(M) / P(T+)",
    solution: `$P(M) = 0.02$, $P(\\bar{M}) = 0.98$, $P(T^+|M) = 0.95$, $P(T^+|\\bar{M}) = 0.03$.

**1)** $P(T^+) = P(T^+|M) \\cdot P(M) + P(T^+|\\bar{M}) \\cdot P(\\bar{M})$
$= 0.95 \\times 0.02 + 0.03 \\times 0.98 = 0.019 + 0.0294 = 0.0484$

**2)** $P(M|T^+) = \\frac{P(T^+|M) \\cdot P(M)}{P(T^+)} = \\frac{0.019}{0.0484} \\approx 39.3\\%$

**3)** حوالي 39% فقط من الإيجابيات هم مرضى فعلاً! 61% إيجابيات كاذبة. هذا يفسر لماذا يطلب الطبيب فحوصات تأكيدية إضافية.`,
  },
];
